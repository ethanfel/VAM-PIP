...
namespace MacGruber
{
    public static class Utils
    {
		...
		public static JSONStorableFloat SetupSliderIntWithRange(MVRScript script, string label, int defaultValue, int minValue, int maxValue, bool rightSide)
		{
			JSONStorableFloat storable = new JSONStorableFloat(label, defaultValue, minValue, maxValue, true, true);
			storable.storeType = JSONStorableParam.StoreType.Full;
			storable.constrained = false;
		...
		public static JSONStorableUrl SetupTexture2DChooser(MVRScript self, string label, string defaultValue, bool rightSide, TextureSettings settings, TextureSetCallback callback, bool infoText = true)
		{
			...
			UIDynamicButton button = self.CreateButton(label, rightSide);
			if(infoText)
            {
		...
		public static UIDynamicTextInfo SetupInfoTextNoScroll(MVRScript script, string text, float height, int fontSize, bool rightSide)
        {
...

			{
				...
				uid.text.text = text;
				uid.text.color = Color.white;
				uid.text.fontSize = fontSize;
		...
		{
			UIDynamicTextInfo uid = SetupInfoTextNoScroll(script, storable.val, height, 32, rightSide);
		...
		{
			UIDynamicTextInfo uid = SetupInfoTextNoScroll(script, text, 35, 32, rightSide);
