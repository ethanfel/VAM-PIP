...
one in-memory undo snapshot before changing the Person and camera. All 19
driven Person controllers and the camera are temporarily kinematic for the
authoritative write. The Person controllers then regain their saved physics
state for an initial settlement window. After that, only `headControl` is
held kinematic at its settled position with the requested rotation, while the
camera remains locked. The Person's original collision setting is restored
immediately after the write. Undo restores the saved transforms and every
saved physics setting; failed Apply and plugin unload also restore every saved
physics state. Undo and capture require the exact currently applied job,
revision, and camera; that state is cleared on undo or plugin restart.
Captures use the renderer's fixed VAM-PIP interface and fixed output name.
